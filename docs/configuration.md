...
| `poisoning_policy` | What the server does about a tool whose name or description trips a [tool-poisoning rule](cli.md#lint): `warn` (default — name it and serve it), `strict` (refuse to start), `off`. The check reads names and descriptions only, so it costs nothing at startup. |
| `auth.introspection` | RFC 7662 token revocation: `url`, `client_id`, `client_secret`, `cache_ttl_s` (default 60 — this is your revocation latency), `fail_open` (default false — refuse when the IdP is unreachable). Bearer tokens only. See [Revocation](transports-and-auth.md#revocation-authintrospection). |
...
| `max_concurrent_calls` | Ceiling on tool calls in flight to upstreams at once, across every caller and both transports (default 256; `0` disables). Past it a call waits for a slot. Distinct from `http.limits.max_in_flight`, which bounds HTTP request *handling* — see [Request caps](transports-and-auth.md#request-caps). |
| `log_hmac_key` | Makes the audit stream tamper-**evident**: each line gets a `seq` and an HMAC chaining it to the previous one, checkable with `minmcp audit-verify` (non-zero exit on a break). A secret reference — a plain hash chain is not evidence, since whoever can rewrite the log can recompute it. See [Tamper evidence](transports-and-auth.md#tamper-evidence-log_hmac_key). |
